...
class CoscmdConfig(object):
    ...
    def __init__(self, appid, region, endpoint, bucket, secret_id, secret_key, token=None,
        ...
        self._suc = success
        logger.debug(
            "config parameter-> appid: {appid}, region: {region}, endpoint: {endpoint}, bucket: {bucket}, part_size: {part_size}, max_thread: {max_thread}".format(
                appid=appid,
                region=region,
                endpoint=endpoint,
                bucket=bucket,
...
class Interface(object):
        ...
        try:
            while IsTruncated == "true":
                url = self._conf.uri(path=quote(to_printable_str(
                    cos_path)) + '?uploadId={UploadId}&upload&max-parts=1000&part-number-marker={nextmarker}'.format(
    ...
    def upload_folder(self, local_path, cos_path, _http_headers='{}', **kwargs):
        ...
        def error_append(err_local_path: str):
            with open(self._conf._error, 'a+') as f:
                f.write(err_local_path)
                f.write(os.linesep)

        def suc_append(suc_local_path: str):
            with open(self._conf._suc, 'a+') as f:
                f.write(suc_local_path)
                f.write('\n')
        ...
        def upload_file_list(upload_filelist):
            ...
            for _local_path, _cos_path in multiupload_filelist:
                ...
                if 0 == rt:
                    _success_num += 1
                    suc_append(_local_path)
                elif -2 == rt:
                    _skip_num += 1
                else:
                    _fail_num += 1
                    error_append(_local_path)
        ...
        if kwargs['sync'] and kwargs['delete']:
            if kwargs['force'] is False and kwargs['yes'] is False:
                if query_yes_no(
                        u"WARN: you are deleting some files in the '{cos_path}' cos_path, please make sure".format(
                            cos_path=raw_cos_path)) is False:
                    return -3
        ...
        def multiupload_parts():
            ...
            def multiupload_parts_data(local_path, offset, length, parts_size, idx):
                try:
                    ...
                    return -1
                url = self._conf.uri(path=quote(to_printable_str(
                    cos_path))) + "?partNumber={partnum}&uploadId={uploadid}".format(partnum=idx,
                                                                                     uploadid=self._upload_id)
                for j in range(self._retry):
                    try:
                        http_header = _http_header
                        rt = self._session.put(url=url,
                                               ...
                                               timeout=self._timeout)
                        logger.debug(
                            "Multi part result: part: {part}, round: {round}, code: {code}, headers: {headers}, text: {text}".format(
                                part=idx,
                                round=j + 1,
                                code=rt.status_code,
                                headers=rt.headers,
                        ...
                        self._session = requests.session()
                        logger.warn(
                            u"Upload part failed, key: {key}, partnumber: {part}, retrytimes: {round}, exception: {error}".format(
        ...
        if kwargs['force'] is False:
            if kwargs['sync'] is True:
                try:
                    ...
                    dst_md5 = "."
                    dst_size = -2
                    rt = self._session.head(url=self._conf._schema + "://" + copy_source['RawPath'],
                                            auth=CosS3Auth(self._conf))
        ...
        try:
            if self._conf._endpoint is not None:
                ...
                if len(source_tmp_path) == 5 and source_tmp_path[1] == 'cos':
                    ...
                    raise Exception("Parse Region Error")
                copy_source['Key'] = source_key
                copy_source['RawPath'] = copy_source['Bucket'] + ".cos." + copy_source['Region'] + ".myqcloud.com/" + \
                                         copy_source['Key']
    ...
    def delete_folder(self, cos_path, **kwargs):
        if kwargs['force'] is False and kwargs['yes'] is False:
            if query_yes_no(u"WARN: you are deleting the file in the '{cos_path}' cos_path, please make sure".format(
                    cos_path=cos_path)) is False:
                return -3
        _force = kwargs['force']
    ...
    def download_folder(self, cos_path, local_path, _http_headers='{}', **kwargs):
        ...
        if kwargs['sync'] and kwargs['delete']:
            if kwargs['force'] is False and kwargs['yes'] is False:
                if query_yes_no(
                        u"WARN: you are deleting the file in the '{local_path}' local_path, please make sure".format(
                            local_path=raw_local_path)) is False:
                    return -3
            logger.info(u"Synchronizing delete, please wait.")
            try:
        ...
        for worker in result['detail']:
            for status in worker[2]:
                if 0 == status:
                    ...
                    _fail_num += 1
        logger.info(
            u"{success_files} files successful, {progress_files} files have in progress, {fail_files} files failed"
            ...
            for grant in grants:
                try:
                    table.add_row(['ACL', ("%s: %s" % (grant.getElementsByTagName("ID")[
                                                           0].childNodes[0].data,
                                                       grant.getElementsByTagName("Permission")[0].childNodes[
                                                           0].data))])
                except Exception:
